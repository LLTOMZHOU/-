...
MINNUM=-1000000
dx=(-100,1,1,0,-1,-1,-1,0,1,0)
dy=(-100,0,1,1,1,0,-1,-1,-1,0)
...
def InitBoard(CastleNum,HellNum,HomeNum):
	...
	for i in range(1,10):#From 1 to 9
			#Board[2][2] is the center of the flag place
...
#
	...
	for i in range(1,BoardHeight+1):
		for j in range(1,BoardWidth+1):
			...
			#If the following situation is satisfied, set the place as Castle
			if rand1 == 1 and rand2 > 10 and rand3 < 2 and Board[i][j].Empty and CastleNum > 0:
				Board[i][j].Name="Castle"
				Board[i][j].Empty=False
				CastleNum-=1
				continue
			#If the following situation is satisfied, set the place as Hell
			...
			#Find a random LUOCHA(vertical distance between the area and the areas surronding) within a range
			Fall=random.randint(6,8)
			#Check the areas around the selected area if the fall(LUOCHA) is too big
			for m in range(1,10):
				if Board[dx[m]+i][dy[m]+j].Empty==False and Board[dx[m]+i][dy[m]+j].Name != "Hell"\
					and Board[dx[m]+i][dy[m]+j].Name != "Castle" and Board[dx[m]+i][dy[m]+j].Name != "Home":
					LUOCHA=Board[dx[m]+i][dy[m]+j].Height-Board[i][j].Height
					#print "   "
					#print Board[dx[m]+i][dy[m]+j].Height, 
					if LUOCHA >= Fall:
						Board[i][j].Height+=random.randint(2, LUOCHA//2.5)
					if LUOCHA <= -Fall:
						Board[i][j].Height-=random.randint(2, -LUOCHA//2.5)
...
def ShowMapPreviewWindow():
	global MapPreview,BackButtonInfo,ACButtonInfo,NewButtonInfo,LoadButtonInfo,Count

	ACButtonInfo=(WindowWidth//3-100, WindowHeight-54, 100, 45, Green, Gold)
	BackButtonInfo=((WindowWidth//3)*2-130, WindowHeight-54, 120, 45, Grey, Red)
	NewButtonInfo=(WindowWidth-150, WindowHeight-54, 90, 45, Red, Blue)
	LoadButtonInfo=(150, 100, 200, 52, Black, Red)
	
	while True:
		NewFlag=False

		#First use the function PRINTBOARD but not update it to the screen
		#So the purpose of this process is only to save the board as a image
		MapPreview=pygame.display.set_mode((WindowWidth-70, WindowHeight-90))
		pygame.display.set_caption("M a p    P r e v i e w")

		global Font
		Font=pygame.font.SysFont("msyh", 80)
		CreateButton(LoadButtonInfo, "Loading......", Font, 13, MapPreview, "English")
		...
		#Reset the content of the MapPreview screen
		...
		while True:
			...
			if MousePressed[0]:
				if ButtonPressed(NewButtonInfo[:4], MousePos):
					time.sleep(0.12)
					NewFlag=True	#Stop showing the current map and generate a new one
				if ButtonPressed(BackButtonInfo[:4], MousePos):
					time.sleep(0.1)
					return		#Normally quit the map viewing procedure
				if ButtonPressed(ACButtonInfo[:4], MousePos):
					if not Saved:
						FileNameForSaving="Map"+str(Count)+".png"
						pygame.image.save(Map, FileNameForSaving)
						Count+=1
						Saved=True   #So that the Map won't be saved twice
			
			#Forced to quit the whole program
...
##########GameUI##############################
def GameUI():
	global UI,CastleNum,HellNum,BoardHeight,BoardWidth,HomeNum
	...
	while True:
		...
		#If the user wants to enter generating mode
		while StartFlag:
			...
			if MousePressed[0]:
				...
				if ButtonPressed(HomeNumAddButtonInfo[:4], MousePos):
					...
					time.sleep(0.12)
				if ButtonPressed(BackButtonInfo[:4], MousePos):
					#Set the flag to False so that the program goes back to the last stage
					StartFlag=False
					time.sleep(0.2)
				if ButtonPressed(GenButtonInfo[:4], MousePos):
					#Set the essential features of the ShowMapPreviewWindow
					global WindowHeight
					global WindowWidth
	
					WindowHeight=BoardHeight*AreaHeight+110
					WindowWidth=BoardWidth*AreaWidth+95
					
					ShowMapPreviewWindow()
	
					#When the Map preview ended, reinit the UI
					UI=pygame.display.set_mode((500,400))
					pygame.display.set_caption("A u t h o r :  T o m")
					UI.fill((25,25,8))
					BackButtonInfo=(320, 320, 120, 40, Grey, Purple)
